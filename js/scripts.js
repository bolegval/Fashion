...
$('.order-item__btn').click(function() {
  let item = $(this).closest('.order-item').find('.order-item__info--id').html();
  $.ajax({
    type: "POST",
...
$('.product-item__delete').click(function() {
  let item = $(this).closest('.product-item').find('.product-item__field--id').html();
...
if (document.querySelector('.shop-page')) {
  ...
  let min = parseInt($('.min-price').text());
  let max = parseInt($('.max-price').text());
  ...
  $('.range__line').slider({
    ...
    slide: function(event, ui) {
      ...
      $('.min-price').text($('.range__line').slider('values', 0) + ' руб.');
      $('.max-price').text($('.range__line').slider('values', 1) + ' руб.');
      $('input[name=minPrice]').val($('.range__line').slider('values', 0));
      $('input[name=maxPrice]').val($('.range__line').slider('values', 1));
    }
  });
}
